...
#### 3f. External Sessions History (v8.0)
```bash
# Quét và lấy session history được liên kết gần nhất
symphony session list --dir . 2>/dev/null
```
Nếu có session gần nhất đã đồng bộ:
→ Đọc summary note từ bảng CSDL `external_sessions`.
→ Đưa summary vào Silent Context để Agent tự động hấp thụ bối cảnh từ công cụ khác (e.g. Claude Code).
...
```json
{
  ...
  "external_session": {
    "id": "e573a153-8c1f-4ed1-a696-82304cd2a3ab",
    "tool_name": "claude-code",
    "summary": "🎼 Handover Note context here..."
  },
  "mindful": {
    "enabled": true,
