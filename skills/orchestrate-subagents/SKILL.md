...
1. **Analyze** - Break the work into discrete, delegatable units and map each unit to the most appropriate runner model ID or alias. Candidates are dynamically selected and prioritized using `model-registry.mjs`.
...
- **Agent type selection (Runner Mapping)** — Specify runner model IDs or aliases. Symphony automatically resolves aliases and builds CLI execution commands dynamically using `model-registry.mjs`:
  - `(qwen)` / `(deepseek)`: Standard logic programming, backend, DB, algorithms (Resolved dynamically to `qwen` / `deepseek`).
  - `(codex)`: UI layouts, styling, graphics, assets generation (Resolved dynamically to `codex`).
  - `(spark)`: Small/local coding tasks for high speed and cost optimization (Resolved dynamically to `gpt-5.3-codex-spark`).
  - `(claude)` / `(fable5)` or `(codex)` / `(gpt-5.6-sol)`: Complex multi-layer logic, system architecture, deep reasoning (Resolved dynamically to `fable` / `gpt-5.6-sol`).
  - `(agy)` / `(flash)` or `(research)`: Diagnostics, exploration, build & test running (Resolved dynamically to `gemini-3.5-flash`).
  - `(critic)` / `(fable5)` / `(gpt-5.6-sol)`: Code quality audits, security scans, edge cases, and final verification approval (Resolved dynamically to `fable` / `gpt-5.6-sol`).
